#!/usr/bin/env bash
# 910B / Atlas A2 GRPO multi-turn tool RL.
# Image: quay.io/ascend/verl:latest-vllm-910b-ubuntu
# SandboxFusion must be reachable at SANDBOX_FUSION_URL (host or sidecar).
set -x
set -euo pipefail
...
# If datasets were placed next to the kit instead: DATA_ROOT=$KIT_ROOT/data/hf
...
experiment_name="${EXPERIMENT_NAME:-qwen2.5-7b_grpo_npu}"
...
python3 -m verl.trainer.main_ppo \
  ...
  actor_rollout_ref.rollout.gpu_memory_utilization="${GPU_MEM:-0.5}" \
